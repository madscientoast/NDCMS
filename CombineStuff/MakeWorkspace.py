...
f = ROOT.TFile("shapes.root", "READ") # I exported the data and original MC to this file. 
data = f.Get("data_obs")
signal = f.Get("signal_m15")

# Create a RooFit workspace and variables
w = ROOT.RooWorkspace("w")

# Define observable (x-axis of the histogram)
mass = ROOT.RooRealVar("x", "x", 178, 1800)

# Create a RooDataHist from the Signal histogram
...
sigma = ROOT.RooRealVar("sigma", "sigma", 10, 5, 15)
...
p0 = ROOT.RooRealVar("p0", "Coefficient", 1.00000e-02, 1e-4, 1)   
...
bkg_expr.fitTo(data_hist, ROOT.RooFit.PrintLevel(-1))
...
mass.setRange("fullRange", 178, 1800)
