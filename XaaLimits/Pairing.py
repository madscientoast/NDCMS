...
def PairDeltaR(photons):
    ...
    return ak.argmin(DR,axis=1), ak.min(DR,axis=1)

def PairMA(photons):
    
    # Compute all possible masses #
    massAB = (photons[:,0]+photons[:,1]).mass
    massAC = (photons[:,0]+photons[:,2]).mass
    massAD = (photons[:,0]+photons[:,3]).mass
    massBC = (photons[:,1]+photons[:,2]).mass
    massBD = (photons[:,1]+photons[:,3]).mass
    massCD = (photons[:,2]+photons[:,3]).mass

    # Compute each MA combination
    MA1 = abs(massAB - massCD)/(massAB + massCD)
    MA2 = abs(massAC - massBD)/(massAC + massBD)
    MA3 = abs(massAD - massBC)/(massAD + massBC)

    MA = ak.concatenate([MA1[:, np.newaxis], MA2[:, np.newaxis], MA3[:,np.newaxis]], axis=1)

    # Find index (0, 1, or 2) of min-ΔR pairing for each event
    return ak.argmin(MA,axis=1)
