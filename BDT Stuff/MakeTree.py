...
#c
with open(base+name+".txt", 'r') as file:
    # Loop through each line in the file
    for line in file:
        # Split by tabs and read as floats
        values = line.strip().split("\t")
        m_avg.append(float(values[1]))
        m_Xm.append(float(values[0]))
        m_asym.append(float(values[5]))
        deltaEta.append(float(values[4]))
        dR1.append(float(values[2]))
        dR2.append(float(values[3]))

# Initialize a ROOT file to save the TTree
output_file = TFile(base+name+".root", "RECREATE")
